...
export async function POST(req: NextRequest) {
  try {
    const webhookSecret = process.env.STRIPE_ELEVATED_WEBHOOK_SECRET;
    const discordWebhookUrl = process.env.DISCORD_STRIPE_ELEVATED_WEBHOOK;

    if (!webhookSecret || !discordWebhookUrl) {
      console.error(
        "Missing STRIPE_ELEVATED_WEBHOOK_SECRET or DISCORD_STRIPE_ELEVATED_WEBHOOK"
      );
    ...
    if (!validSignature) {
      console.error("Invalid Elevated Stripe webhook signature.");
    ...
    const message = isFailed
      ? `❌ ELEVATED STRIPE PAYMENT DECLINED
...
🕘 Time: ${formatMelbourneDateTimeFromUnix(charge.created)}`;
    ...
    if (!discordRes.ok) {
      ...
      console.error(
        "Failed to send Elevated Stripe Discord notification:",
        text
      );
    ...
    return Response.json({
      success: true,
      message: isFailed
        ? "Elevated Stripe declined payment notification sent."
        : "Elevated Stripe successful payment notification sent.",
    });
  } catch (error) {
    console.error("Elevated Stripe webhook failed:", error);
